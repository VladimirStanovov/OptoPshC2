...
int init_generator = 1;
int GNVars;

double RandRotMatrix_17[30][10][100][100];
double RandShiftMatrix_17[30][10][100];
double RandShuffleMatrix_17[30][10][100];
...
void cec17_test_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	if (ini_flag==0)
	{
	    if(init_generator == 0)
		{
            ...
            /* Load Matrix M*/
            sprintf(FileName, "%s/M_%d_D%d.txt",path_cec, func_num,nx);
            ...
            /* Load shift_data */
            sprintf(FileName, "%s/shift_data_%d.txt",path_cec, func_num);
            fpt = fopen(FileName,"r");
            if (fpt==NULL)
            {
                printf("\n Error: Cannot open input file for reading \n");
            ...
            if (func_num<20)
            {
                ...
                for(i=0;i<nx;i++)
                {
                    tmpfscf = fscanf(fpt,"%lf",&OShift[i]);
                }
            }
            else
            {
                ...
                for(i=0;i<cf_num-1;i++)
                {
                    for (j=0;j<nx;j++)
                    {
                        tmpfscf = fscanf(fpt,"%lf",&OShift[i*nx+j]);
                    }
                    tmpfscf = fscanf(fpt,"%*[^\n]%*c");
                }
                for (j=0;j<nx;j++)
                {
                    tmpfscf = fscanf(fpt,"%lf",&OShift[(cf_num-1)*nx+j]);
                }
            ...
            {
                free(SS);
                sprintf(FileName, "%s/shuffle_data_%d_D%d.txt",path_cec, func_num, nx);
                fpt = fopen(FileName,"r");
                if (fpt==NULL)
                {
                    printf("\n Error: Cannot open input file for reading \n");
		...
		else
        {
            ...
            if (func_num<20)
            {
                ...
                for(int i2=0;i2!=nx;i2++)
                {
                    ...
                    {
                        M[counter] = RandRotMatrix_17[func_num-1][0][i2][j2];
                        counter++;
                    }
                }
            }
            else
            {
                ...
                if (M==NULL)
                    printf("\nError: there is insufficient memory available!\n");
                int counter = 0;
                for(int k=0;k!=cf_num;k++)
                {
                    for(int i2=0;i2!=nx;i2++)
                    {
                        for(int j2=0;j2!=nx;j2++)
                        {
                            M[counter] = RandRotMatrix_17[func_num-1][k][i2][j2];
                            counter++;
                        }
                    }
                }
            ...
            if (func_num<20)
            {
                ...
                for(i=0;i<nx;i++)
                {
                    OShift[i] = RandShiftMatrix_17[func_num-1][0][i];
                }
            }
            else
            {
                ...
                for(i=0;i<cf_num;i++)
                {
                    for (j=0;j<nx;j++)
                    {
                        OShift[counter] = RandShiftMatrix_17[func_num-1][i][j];
                        counter++;
                    }
                }
            }
            ...
            if (func_num>=11&&func_num<=20)
            {
                ...
                for(i=0;i<nx;i++)
                {
                    SS[i] = RandShuffleMatrix_17[func_num-1][0][i];
                }
            }
            else if (func_num==29||func_num==30)
            {
                ...
                for(i=0;i<cf_num;i++)
                {
                    for (j=0;j<nx;j++)
                    {
                        SS[counter] = RandShuffleMatrix_17[func_num-1][i][j];
                        counter++;
                    }
                }
            }
...
void sum_diff_pow_func (double *x, double *f, int nx, double *Os,double *Mr, int s_flag, int r_flag) /* sum of different power */
{
	...
	for (i=0; i<nx; i++)
	{
		...
		double newv = fast_pow((fabs(xi)),(i+1));
		sum = sum + newv;
	}
...
void zakharov_func (double *x, double *f, int nx, double *Os,double *Mr, int s_flag, int r_flag) /* zakharov */
{
	...
	for (i=0; i<nx; i++)
	{
		double xi = z[i];
		//sum1 = sum1 + pow(xi,2);
		sum1 = sum1 + fast_pow(xi,2);
		sum2 = sum2 + 0.5*(i+1)*xi;
	}

	//f[0] = sum1 + pow(sum2,2) + pow(sum2,4);
	f[0] = sum1 + fast_pow(sum2,2) + fast_pow(sum2,4);
}
...
void levy_func (double *x, double *f, int nx, double *Os,double *Mr, int s_flag, int r_flag) /* Levy */
{
	...
	double term1 = fast_pow((sin(PI*w[0])),2);
	//double term3 = pow((w[nx-1]-1),2) * (1+pow((sin(2*PI*w[nx-1])),2));
	double term3 = fast_pow((w[nx-1]-1),2) * (1+fast_pow((sin(2*PI*w[nx-1])),2));
	...
	for (i=0; i<nx-1; i++)
	{
		...
		double newv = fast_pow((wi-1),2) * (1+10*fast_pow((sin(PI*wi+1)),2));
		sum = sum + newv;
	}
...
void dixon_price_func (double *x, double *f, int nx, double *Os,double *Mr, int s_flag, int r_flag) /* Dixon and Price */
{
	...
	for (i=1; i<nx; i++)
	{
		...
		double newv = i * fast_pow((fast_pow(2*xi,2) - xold),2);
		sum = sum + newv;
	}
...
void weierstrass_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Weierstrass's  */
{
	...
	for (i=0; i<nx; i++)
	{
		...
		for (j=0; j<=k_max; j++)
		{
			...
			sum += fast_pow(a,j)*cos(2.0*PI*fast_pow(b,j)*(z[i]+0.5));
			//sum2 += pow(a,j)*cos(2.0*PI*pow(b,j)*0.5);
			sum2 += fast_pow(a,j)*cos(2.0*PI*fast_pow(b,j)*0.5);
		}
		f[0] += sum;
	}
	f[0] -= nx*sum2;
...
/* original global optimum: [-1,-1,...,-1] */
{
	...
	f[0]=sqrt(fabs(fast_pow(r2,2)-fast_pow(sum_z,2))) + (0.5*r2 + sum_z)/nx + 0.5;
}
...
void cf09 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int r_flag)
{
	...
	double bias[3] = {0, 100, 200};
	i=0;
	hf05(x,&fit[i],nx,&Os[i*nx],&Mr[i*nx*nx],&S[i*nx],1,r_flag);
	i=1;
	hf06(x,&fit[i],nx,&Os[i*nx],&Mr[i*nx*nx],&S[i*nx],1,r_flag);
	i=2;
	hf07(x,&fit[i],nx,&Os[i*nx],&Mr[i*nx*nx],&S[i*nx],1,r_flag);
	cf_cal(x, f, nx, Os, delta,bias,fit,cf_num);

}

void cf10 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int r_flag)
{
	int i,cf_num=3;
	double fit[3];
	double delta[3] = {10,30,50};
	...
	hf05(x,&fit[i],nx,&Os[i*nx],&Mr[i*nx*nx],&S[i*nx],1,r_flag);
	i=1;
	hf08(x,&fit[i],nx,&Os[i*nx],&Mr[i*nx*nx],&S[i*nx],1,r_flag);
	i=2;
	hf09(x,&fit[i],nx,&Os[i*nx],&Mr[i*nx*nx],&S[i*nx],1,r_flag);
	cf_cal(x, f, nx, Os, delta,bias,fit,cf_num);
}
...
void cf_cal(double *x, double *f, int nx, double *Os,double * delta,double * bias,double * fit, int cf_num)
{
	...
	for (i=0; i<cf_num; i++)
	{
		...
		for (j=0; j<nx; j++)
		{
			...
			w[i]+=fast_pow(x[j]-Os[i*nx+j],2);
		}
		if (w[i]!=0)
			//w[i]=pow(1.0/w[i],0.5)*exp(-w[i]/2.0/nx/pow(delta[i],2.0));
			w[i]=pow(1.0/w[i],0.5)*exp(-w[i]/2.0/nx/fast_pow(delta[i],2));
		else
			w[i]=INF;
		if (w[i]>w_max)
			w_max=w[i];
